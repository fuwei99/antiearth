...
router.post('/tokens/import', cookieAuthMiddleware, async (req, res) => {
  ...
  const tokensToImport = data?.tokens || (Array.isArray(data) ? data : null);
  if (!tokensToImport) {
    return res.status(400).json({ success: false, message: '无效的导入数据格式' });
  }

  try {
    const { added, updated, skipped } = await tokenManager.importTokens(tokensToImport, mode);

    res.json({
      success: true,
      message: `导入完成：新增 ${added} 个，更新 ${updated} 个，跳过 ${skipped} 个`,
