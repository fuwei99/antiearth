...
function logRequest(method, path, status, duration, extra = '') {
  const statusColor = status >= 500 ? colors.red : status >= 400 ? colors.yellow : colors.green;
  const message = `[${method}] - ${path} ${status} ${duration}ms${extra ? ' ' + extra : ''}`;

  console.log(`${colors.cyan}[${method}]${colors.reset} - ${path} ${statusColor}${status}${colors.reset} ${colors.gray}${duration}ms${colors.reset}${extra ? ' ' + extra : ''}`);
