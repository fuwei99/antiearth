  ...
  while (true) {
    try {
      ...
      if (status === 429 && !hint.hasRetryHint) {
        logger.info(`${loggerPrefix}收到 429，错误响应体未提供等待间隔，将尝试轮换Token重试`);
      }

      if (status === 429) {
        const tokenId = getCurrentTokenId(retryOptions);
        if (tokenId && modelId) {
          let cooldownDuration = 5 * 60 * 1000;
          if (hint.explicitDelayMs && hint.explicitDelayMs > 0) {
            cooldownDuration = hint.explicitDelayMs;
          } else {
            const bodyText = stringifyErrorBody(hint.body, error?.message);
            if (bodyText.includes('INSUFFICIENT_G1_CREDITS_BALANCE') || bodyText.includes('RESOURCE_EXHAUSTED')) {
